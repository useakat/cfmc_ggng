...
while [ $j -ne $nplot ];do
	...
	im1=`expr $i - 1`
	echo "'rslt_${input[$im1]}/data/${plot_wgt[$j]}.dat' u (\$1+${xshift[$j]}):2 t '${input[$im1]}_{wgt}' w histep lt $i lw 4, \\" >> $gnu_file
	echo "'rslt_${input[$im1]}/data/${plot_wgt[$j]}.dat' u (\$1+${xshift[$j]}):(\$2+\$3*2) notitle w histep lt $i+1 lw 1, \\" >> $gnu_file
	echo "'rslt_${input[$im1]}/data/${plot_wgt[$j]}.dat' u (\$1+${xshift[$j]}):(\$2-\$3*2) notitle w histep lt $i+1 lw 1, \\" >> $gnu_file
	echo "'Events/${input[$im1]}/${plot_unwgt[$j]}.dat' u 1:(\$2/${binsize[$j]}) t '${input[$im1]}_{unwgt}' w histep lt $i lw 2, \\" >> $gnu_file
	echo "'Events/${input[$im1]}/${plot_unwgt[$j]}.dat' u 1:((\$2+\$3)/${binsize[$j]}) notitle w histep lt $i+2 lw 1, \\" >> $gnu_file
	echo "'Events/${input[$im1]}/${plot_unwgt[$j]}.dat' u 1:((\$2-\$3)/${binsize[$j]}) notitle w histep lt $i+2 lw 1, \\" >> $gnu_file
	i=`expr $i + 1`
    done
    echo "'rslt_${input[0]}/data/${plot_wgt[$j]}.dat' every::0::0 notitle w histep lt 1 lw 0" >> $gnu_file
