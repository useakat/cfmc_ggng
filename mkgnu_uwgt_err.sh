...
if [[ "$1" == "-h" ]]; then
    echo ""
    echo "Usage: mkgnu.sh [run] [run_me]"
    echo ""
    exit
fi
if [[ $1 = "" ]]; then
...
#    echo "'Events/${run}/${plot_unwgt[$j]}.dat' u 1:(\$2/${binsize[$j]}) t '${run}_{unwgt}' w histep lt 1 lc rgb 'red' lw 3, \\" >> $gnu_file
    echo "'Events/${run}/${plot_unwgt[$j]}.dat' u 1:(\$2/${binsize[$j]}) t 'LC ch' w histep lt 1 lc rgb 'red' lw 3, \\" >> $gnu_file
    echo "'Events/${run}/${plot_unwgt[$j]}.dat' u 1:((\$2+\$3)/${binsize[$j]}) notitle w histep lt 2 lc rgb 'red' lw 1, \\" >> $gnu_file
    echo "'Events/${run}/${plot_unwgt[$j]}.dat' u 1:((\$2-\$3)/${binsize[$j]}) notitle w histep lt 2 lc rgb 'red' lw 1, \\" >> $gnu_file
#    echo "'Events/${run_me}/${plot_unwgt[$j]}.dat' u 1:(\$2/${binsize[$j]}) t '${run_me}_{unwgt}' w histep lt 1 lc rgb '#006400' lw 3, \\" >> $gnu_file
    echo "'Events/${run_me}/${plot_unwgt[$j]}.dat' u 1:(\$2/${binsize[$j]}) t 'MG LC' w histep lt 1 lc rgb '#006400' lw 3, \\" >> $gnu_file
    echo "'Events/${run_me}/${plot_unwgt[$j]}.dat' u 1:((\$2+\$3)/${binsize[$j]}) notitle w histep lt 2 lc rgb '#006400' lw 1, \\" >> $gnu_file
    echo "'Events/${run_me}/${plot_unwgt[$j]}.dat' u 1:((\$2-\$3)/${binsize[$j]}) notitle w histep lt 2 lc rgb '#006400' lw 1" >> $gnu_file
