...
c       include 'inc/const.inc'
       ...
       real*8 phi,ETij,gamma,gambeta,D
       real*8 esbeta,es4sq
...
c         PTcut2 = 2*PTcut -M(2,n+1)*dsin(phi)**2
         PTcut2 = 2*Ptcut
       ...
       ETij = dsqrt(M(2,50) +KPT2(50))
       gamma = ETij/M(1,50)
       gambeta = KPT(50)/M(1,50)
c       D = (gamma**2 -dsin(phi)**2)*( 4*PTcut**2/M(2,50) -dsin(phi)**2 ) 
c     &      +gambeta**2*dsin(phi)**2
c       if (D.lt.0) THEN
c          write(99,*) "ERROR: D is negative.."
c          stop
c       ENDIF  
c       costhcut_kt = ( gamma*gambeta -dsqrt(D))/(gamma**2 -dsin(phi)**2) 
       costhcut = gambeta/gamma -2*PTcut/( M(1,50)*gamma )
       mincosth = -costhcut
       maxcosth = costhcut
       costh = mincosth +(maxcosth -mincosth)*rdn2(1)
       wpsn3 = wpsn3*(maxcosth -mincosth)
       jacobn3 = 1d0

       beta = esbeta(mmass(1)**2/M(2,50),mmass(2)**2/M(2,50))
       sinth = dsqrt(1d0 -costh**2)
c       call mom2cx(sqrts,mass(1),mass(2),costh,phi, PPP(0,1),PPP(0,2))
       PPP(0,1) = M(1,50)/2d0*(1d0 +(mmass(1)**2 -mmass(2)**2)/M(2,50))
       PPP(1,1) = beta*costh
       PPP(2,1) = beta*sinth*dcos(phi)
       PPP(3,1) = beta*sinth*dsin(phi)
       PPP(0,2) = M(1,50)/2d0*(1d0 +(mmass(2)**2 -mmass(1)**2)/M(2,50))
       PPP(1,2) = -beta*costh
       PPP(2,2) = -beta*sinth*dcos(phi)
       PPP(3,2) = -beta*sinth*dsin(phi)

       call boostx(PPP(0,1),P(1,50),PP(0,1))
       call boostx(PPP(0,2),P(1,50),PP(0,2))
