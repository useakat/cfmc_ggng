...
c       DOUBLE PRECISION cuteta2,cuteta3
...
c     KETA(n) = -cuteta +2*cuteta*rdn(3*n-2)
c     jjacob(3*n-2) = 1d0
c     wwgt(3*n-2) = 2*cuteta
...
c----------------------------------------------------------------------------------------------
       SUBROUTINE pshk_poslastp(itype,SS,n,rdn,x1,x2,WPSn,Jacob,IFLG)
       DOUBLE PRECISION P(5,50),M(3,50)
       COMMON /PARTCL/P,M
       DOUBLE PRECISION KPT2(20),KETA(20),KPhi(20),KPT(20),DKETA(20)
       REAL*8 PTcut,AYEcut,QCut,Pi,Qmax
...
c       DOUBLE PRECISION cuteta2,cuteta3
       ...
       if (itype.gt.0) then ! Exchange the n-th and (n-itype)-th momentum.
          do i = 1,3
             M_tmp(i) = M(i,2+n-itype)
             M(i,2+n-itype) = M(i,2+n)
             M(i,2+n) = M_tmp(i)
          enddo
       endif
...
c     &         ayecut, dacosh( dsqrt( ss/( M(2,n+2) +KPT2(n) ) ) ) )
     &            ayecut, dacosh( dsqrt( ss/( M(2,n+2) ) ) ) )
       ...
       if (itype.gt.0) then  ! The (n-itype)-th momentum is generated last.
          do i = 1,3
             M_tmp(i) = M(i,2+n-itype)
             M(i,2+n-itype) = M(i,2+n)
             M(i,2+n) = M_tmp(i)
          enddo
          KPT2_tmp = KPT2(n-itype)
          KPT2(n-itype) = KPT2(n)
          KPT2(n) = KPT2_tmp
          KPT(n-itype) = dsqrt(KPT2(n-itype))
          KPT(n) = dsqrt(KPT2(n))
          KPhi_tmp = KPhi(n-itype)
          KPhi(n-itype) = KPhi(n)
          KPhi(n) = KPhi_tmp
          cuteta_tmp = cuteta(n-itype)
          cuteta(n-itype) = cuteta(n)
          cuteta(n) = cuteta_tmp
       endif
...
c     wwgt(3*n-2) = 2*cuteta       
     ...
     &      *0.38937966d9
       ...
       SUBROUTINE pshk_m(SS,n,rdn,ipw12,x1,x2,WPSn,Jacob,IFLG)
       DOUBLE PRECISION P(5,50),M(3,50)
       COMMON /PARTCL/P,M
       ...
       COMMON /cut2/PTcut2,AYEcut2,AYEcut3
       COMMON /PARTCL2/ KPT2, KETA, KPhi,KPT
       INTEGER I,J,IFLG,nsch
       DOUBLE PRECISION rdn(30),KEPT2(20)
       DOUBLE PRECISION spt1,spt2,E00,PZ0,cutpt,cutpt2,cuteta,cuteta2
       DOUBLE PRECISION cuteta3
       DOUBLE PRECISION SS,sh,x1,x2,pipt2,WPSn,pau2,Jacob
       double precision jjacob(3*n-2+n),wwgt(3*n-2+n)
       real*8 ipw1,ipw2,mini2,max2,mmx,ipw12
       real*8 ptmaxl,ptminl,aa
       ...
       do i = 1,n-1
          ...
          if ((M(3,i+2).eq.1).or.(M(1,i+2).gt.0d0)) then
             ...
             if (M(3,i+2).eq.1) then
                nsch = nsch +1
c                mini2 = max( Qcut**2, 4*ptcut**2*( 1d0 -KPT(i)/ptcut ) )
                mini2 = Qcut**2
                max2 = Qmax**2
                mmx = dlog(mini2) +( dlog(max2) -dlog(mini2) )*rdn(3*n-2+nsch)     
                M(2,i+2) = dexp(mmx)
                M(1,i+2) = dsqrt(M(2,i+2))
                wwgt(3*n-2+nsch) = ( dlog(max2) -dlog(mini2) )/2d0/pi
                jjacob(3*n-2+nsch) = dexp(mmx)
...
c     &               ,dacosh( dsqrt( ss/( M(2,i+2) +KPT2(i) ) ) ))
     &               ,dacosh( dsqrt( ss/( M(2,i+2) ) ) ))
       ...
       if (M(3,n+2).eq.1) then
          nsch = nsch +1
c          mini2 = max(Qcut**2,4*ptcut**2*(1d0-KPT(n)/ptcut))
          mini2 = Qcut**2
          max2 = Qmax**2
          mmx = dlog(mini2) +( dlog(max2) -dlog(mini2) )*rdn(3*n-2+nsch)     
          M(2,n+2) = dexp(mmx)
...
c==============================================================
       SUBROUTINE pshk2_2_0(SS,n,rdn,x1,x2,WPSn,Jacobn,IFLG)
       DOUBLE PRECISION P(5,50),M(3,50)
       COMMON /PARTCL/P,M
       REAL*8 PTcut,AYEcut,QCut,Pi,Qmax
       COMMON /cut/PTcut,AYEcut,QCut,Pi,Qmax
...
c==============================================================
       SUBROUTINE pshk2_2_m(SS,n,rdn,ipw12,x1,x2,WPSn,Jacobn,IFLG)
       DOUBLE PRECISION P(5,50),M(3,50)
       COMMON /PARTCL/P,M
       REAL*8 PTcut,AYEcut,QCut,Pi,Qmax
       COMMON /cut/PTcut,AYEcut,QCut,Pi,Qmax
       ...
       DOUBLE PRECISION Jacobn,Jacobn1,Jacobn2,Jacobn3,Jacobn4
       DOUBLE PRECISION mtemp1_1,mtemp1_2,mtemp2_1,mtemp2_2,mmx,mmx1
       double precision mmx2,mini2,max2,mmass(4),ipw12
       ...
       if(n.ge.4) then
          ...
          CALL pshk_m(SS,n-2,rdn,ipw12,x1,x2,WPSn2,Jacobn2,IFLG)
          IF(IFLG.ne.0) then
             return
          endif
       else
...
c       CALL DK2(49,n-1,n,rdn3,WPSn4,Jacobn4,IFLG3) 
     ...
     &      ,iflg3)
       ...
       WPSn=WPSn2*WPSn3*WPSn4
       Jacobn=Jacobn2*Jacobn3*Jacobn4
