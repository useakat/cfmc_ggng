...
c         ptsqmin = pt**2
c         if (pt0.lt.pt) ptsqmin = pt0**2
c         rr = cosh(eta-eta0)-0.5d0*ptcut**2/ptsqmin
c         r = 0.d0
c         if (rr.le.1.d0) r = acos(rr)
c         phi = phi0 + r + 2.d0*(pi-r)*rdn((2*n-1)+i)
c         if (phi.gt.2.d0*pi) phi = phi - 2.d0*pi
         phi = phi0 + 2.d0*pi*rdn((2*n-1)+i)
c         jacob = jacob * 2.d0*(pi-r) * (eta0+etacut)
         jacob = jacob * 2.d0*pi * (eta0+etacut)
         psumx = psumx-pt*cos(phi)
         psumy = psumy-pt*sin(phi)
c         p(0,i+2) = 0.5d0*pt*(exp(eta)+exp(-eta))
c         p(3,i+2) = 0.5d0*pt*(exp(eta)-exp(-eta))
